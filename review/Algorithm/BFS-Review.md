...
1. 整体做法和207一样，但是在queue.poll时，需要记录当前的课程
2. 如果都能修完，直接返回记录的数组，否则返回空数组
...
1. BFS 搜索最短路径比较合适 ： 最短的步数， 最少交换次数。BFS搜索过程中遇到的解一定是离根最近的，所以遇到一个解，一定是最优解。 DFS不一定能搜索到最近的解
2. DFS空间效率高。 DFS不需要保存搜索过程中的状态。BFS需要保存搜索过程中的状态。
3. DFS适合搜索全部的解，
4. 可以使用双向BFS优化搜索速度